"""
Idea:
Dividing array in half, in each iteration
-checking if books can be divided in each student, if yes, finding the max book pages & returning
-returning the minimum of total returns
"""
...
class Solution:
    ...
    def isPossible(self, arr, n, m, curr_min):
        ...
        for i in range(n):
            ...
            if curr_sum + arr[i] > curr_min:
                ...
                # if students required becomes greater than given number of students, we return false.
    ...
    def findPages(self, arr, n, m):
        ...
        while start <= end:
            ...
            # checking if it is possible to distribute books by using mid as current minimum.
            if self.isPossible(arr, n, m, mid):

                # if yes, then we find the minimum distribution.
                result = min(result, mid)

                # as we are finding minimum and books are sorted so reducing end as end = mid -1.
                end = mid - 1

            # if not possible means pages should be increased so updating start = mid + 1.
            else:
                start = mid + 1
