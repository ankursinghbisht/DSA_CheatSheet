...
class Solution {
public:
    int count, ans = -1; // Variables to keep track of count and the answer

    // Function to recursively find the Kth smallest element in the BST
    void solve(Node* root) {
        if (root) {
            solve(root->left); // Traverse left subtree
            --count; // Decrement count
            if (!count) // If count becomes 0, assign the current node's data to ans
                ans = root->data;
    ...
    int KthSmallestElement(Node* root, int K) {
        count = K; // Initialize count to K
        solve(root); // Call the recursive function to find the Kth smallest element
