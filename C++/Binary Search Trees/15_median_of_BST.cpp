...
{
    // this function keeps track of serial number while doing  inorder traveresal and finds the target node

    if (!n) return serialNo;
    serialNo = find(n->left, serialNo, target, value);

    serialNo++;
    // current value of serialNo variable is the rank of  current Node
...
float findMedian(struct Node* root)
{
    ...
    if (n % 2)
    {
        // odd number of nodes in tree:  1+n/2 is the rank of mid node
        int mid = 0;
    ...
    {
        // even number of nodes in tree:  n/2 and 1+n/2 are the ranks of mid nodes
        int mid1 = 0, mid2 = 0;
