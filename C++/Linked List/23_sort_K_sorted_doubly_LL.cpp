/*
Idea:
Using priority queue to store all nodes according to their values, and
popping top priority node and updating it's links
*/
...
struct Node* sortAKSortedDLL(struct Node* head, int k)
{
    ...
    while (!pq.empty()) {
        ...
        // result sorted list so far having the first node pointed to by 'newHead' and adjust the required links
        if (newHead == NULL) {
            newHead = pq.top();
            newHead->prev = NULL;
